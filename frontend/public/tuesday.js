...
function name_block_update(){
        ...
        if(arr_dialog.choice){
            ...
            for(i=0;i < arr_dialog.choice.length;i++){
                ...
                if (arr_dialog.choice[i].go_to){
                    ...
                    else if (g == "tue_load_autosave"){choice.setAttribute("onclick",v+"load_stag('auto');"+add_sound());  if(story_json.parameters.autosave && !localStorage.getItem("tue_auto_data")){choice.style.visibility='hidden'}}
    ...
    var res = "";
    var lang_val = data[languare];
    // Check if lang_val is defined and not null (allow 0 and empty string to pass through for now to see what's happening)
    if (lang_val !== undefined && lang_val !== null){
        if(typeof lang_val === 'number'){
             // 1-based index check
             if(story_json.base && story_json.base[lang_val-1]) res = story_json.base[lang_val-1];
             else console.error("TuesdayJS: Base64 index not found for lang " + languare + ":", lang_val, "(Note: Index should be 1-based)");
        }
        else {res = lang_val}
    } else {
        if(typeof data === 'number'){
             if(story_json.base && story_json.base[data-1]) res = story_json.base[data-1];
             else console.error("TuesdayJS: Base64 index not found:", data);
        }
        else if(typeof data === 'object'){
             var keys = Object.keys(data);
             if(keys.length > 0){
                 var fallbackKey = keys[0];
                 var val = data[fallbackKey];
                 if(typeof val === 'number'){
                     if(story_json.base && story_json.base[val-1]) res = story_json.base[val-1];
                 } else { res = val; }
                 
                 console.warn("TuesdayJS: Missing translation for '" + languare + "'. Data type for lang:", typeof lang_val, "Value:", lang_val);
                 console.warn("TuesdayJS: Using fallback key:", fallbackKey, "Value (truncated):", (typeof res === 'string' && res.length > 50 ? res.substring(0, 50) + "..." : res));
                 console.log("TuesdayJS: Full Data Object:", data); // Log full object to inspect
             }
        }
        else {res = data}
    }
    
    if(typeof res === 'string' && res.length > 50 && !res.startsWith("data:") && !res.startsWith("http") && !res.startsWith("./") && !res.startsWith("/")) {
        // Only warn if it really looks like a broken path, NOT if it's just a short relative path. 
        // 50 chars is long for a filename but short for base64.
        console.warn("TuesdayJS: Image data might be missing 'data:image/...' prefix or is invalid URL:", res.substring(0, 50) + "...");
    }
    return res;
} function video_size(){
